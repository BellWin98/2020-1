...
int main() {
	...
	// Labelling
	// Binarization(Image, Output, hinfo.biWidth, hinfo.biHeight, 40);
	// m_BlobColoring(Output, H, W);

	// Edge검출
	Binarization(Image, Temp, W, H, 40); // 경계 검출시 사용. pupil영상은 Threshold 50이 적당.
	...
	SaveBMPFile(hf, hinfo, hRGB, Output, hinfo.biWidth, hinfo.biHeight, "test_bin_pupil.bmp");
	free(Image);
	free(Output);
	return 0;
}
