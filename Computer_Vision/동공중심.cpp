...
void HighPassFilter2(BYTE * Image, BYTE * Output, int* xm, int* ym,
	const int S, int W, int H) // 마스크 합이 1인 경우 사용할 함수
{
	int * Temp = (int *)malloc(W*H * sizeof(int));
	int * TempX = (int *)malloc(W*H*sizeof(int));
	int * TempY = (int *)malloc(W*H * sizeof(int));
	int Margin = SIZE / 2;
	double Maskx[SIZE][SIZE];
	double Masky[SIZE][SIZE];
	//int SobelX[] = { 0 };
	//int SobelY[] = { 0 };
	for (int i = 0; i < SIZE*SIZE; i++)
		Maskx[i / SIZE][i%SIZE] = xm[i];
	for (int i = 0; i < SIZE*SIZE; i++)
		Masky[i / SIZE][i%SIZE] = ym[i];
	...
	for (int i = Margin; i < H - Margin; i++) { // 마스크 중앙의 세로방향 이동
		for (int j = Margin; j < W - Margin; j++) { // 마스크 중앙의 가로방향 이동
			for (int m = -Margin; m <= Margin; m++) { // 마스크 중앙 기준 세로방향 주변화소 접근
				for (int n = -Margin; n <= Margin; n++) { // 마스크 중앙 기준 가로방향 주변화소 접근
					temp += (Image[(i + m)*W + (j + n)] * Maskx[m + Margin][n + Margin]); //convolution 연산
				}
			}
	...
	for (int i = Margin; i < H - Margin; i++) { // 마스크 중앙의 세로방향 이동
		for (int j = Margin; j < W - Margin; j++) { // 마스크 중앙의 가로방향 이동
			for (int m = -Margin; m <= Margin; m++) { // 마스크 중앙 기준 세로방향 주변화소 접근
				for (int n = -Margin; n <= Margin; n++) { // 마스크 중앙 기준 가로방향 주변화소 접근
					temp += (Image[(i + m)*W + (j + n)] * Masky[m + Margin][n + Margin]);
				}
			}
			if (temp > Max) Max = temp;
			if (temp < Min) Min = temp;
	...
	for (int i = Margin; i < H - Margin; i++) {
		for (int j = Margin; j < W - Margin; j++) {
			if (TempX[i*W + j] > TempY[i*W + j])
				Temp[i*W + j] = TempX[i*W + j];
			else
				Temp[i*W + j] = TempY[i*W + j];
		}
	}
	for (int i = 0; i < W*H; i++)
...
void m_BlobColoring(BYTE* CutImage, int height, int width)
{
	...
	// 가장 면적이 넓은 영역을 찾아내기 위함 .가장 큰 덩어리를 찾는 코드
...
void main()
{
	...
	/*int sxm[SIZE*SIZE] = { -1, 0, 1,
						   -2, 0, 2,
						   -1, 0, 1 };
	int sym[SIZE*SIZE] = { -1,-2,-1,
						   0, 0, 0,
						   1, 2 ,1 };
	HighPassFilter2(Image, Output, sxm, sym, SIZE, hInfo.biWidth, hInfo.biHeight);*/
	//BYTE temp[9]; //나중에 함수로 분리하기
	int W = hInfo.biWidth, H = hInfo.biHeight;
	/*
	int Margin = SIZE / 2;
	for (int i = Margin; i < H - Margin; i++) {
		for (int j = Margin; j < W - Margin; j++) {
			temp[0] = Image[(i-1)*W + j-1]; //j-1내 왼쪽 i-1내 위쪽 i+1 내 아래쪽
			temp[1] = Image[(i-1)*W + j];
			temp[2] = Image[(i-1)*W + j+1];//j+1 내 오른쪽
	...
	for (int i = 0; i < H; i++)
		Image[i*W + x / count] = 255;
	for (int j = 0; j < W; j++)
		Image[y / count * W + j] = 255;
	...
	//for (int i = 1; i < H; i++) {
	//	for (int j = 1; j < W; j++) {
	//		if (Output[i*W + j] == 255) { //전경 화소라면
	//			if (!(Output[(i + 1)*W + j] == 255 && //위
	//				Output[(i - 1)*W + j] == 255 && //아래
	//				Output[i*W + (j + 1)] == 255 && //우
	//				Output[i*W + (j - 1)] == 255)) { //좌
	//				Image[i*W + j] == 255; //경계 하얗게
	//			}
	//		}
	//	}
	//}
	...
	fwrite(Image, sizeof(BYTE), ImgSize, fp); //원래 Output
	fclose(fp);
	free(Image);
	free(Output);
}